...
(function($){
    ...
    var arrowUpN            = '../common/html5movie/img/arrow_up_n.png';       // △画像名（ノーマル）
    var arrowDownN          = '../common/html5movie/img/arrow_down_n.png';     // ▽画像名（ノーマル）
    var arrowUpG            = '../common/html5movie/img/arrow_up_g.png';       // △画像名（非選択）
    var arrowDownG          = '../common/html5movie/img/arrow_down_g.png';     // ▽画像名（非選択）

    var status              = 0;                    // ステータス
    var statusPlay          = 1;                    // 再生中
    var statusPause         = 2;                    // 一時停止中
    var statusStop          = 3;                    // 停止中
    var playBtnImgN         = '../common/html5movie/img/btn_play.png';       // 再生ボタン画像
    var playBtnImgO         = '../common/html5movie/img/btn_play_o.png';     // 再生ボタン画像(hover)
    var playBtnImgOTab      = '../common/html5movie/img/btn_tab_play_o.png'; // 再生ボタン画像(hover,tablet)
    var pauseBtnImgN        = '../common/html5movie/img/btn_pause.png';      // 一時停止ボタン画像
    var pauseBtnImgO        = '../common/html5movie/img/btn_pause_o.png';    // 一時停止ボタン画像(hover)
    var pauseBtnImgOTab     = '../common/html5movie/img/btn_tab_pause_o.png';// 一時停止ボタン画像(hover,tablet)
    var stopBtnImgN         = '../common/html5movie/img/btn_stop.png';       // 停止ボタン画像
    var stopBtnImgO         = '../common/html5movie/img/btn_stop_o.png';     // 停止ボタン画像(hover)
    var stopBtnImgOTab      = '../common/html5movie/img/btn_tab_stop_o.png'; // 停止ボタン画像(hover,tablet)

    var titleNameSp         = 'h1';                         // タイトル(SP)
    var chapterListNameSp   = 'section :first';             // チャプターリスト(SP)
    var posterNoImage       = 'sp_images/no_img.jpg';   // posterのno image
...
//            playerObj.after(subtitleWrap);
    ...
    function getConvertHour(time) {
        hour = (time / 3600);
        if(hour >= 1) return Math.round(hour);
        return 0;
    }
    ...
    function getConvertMinute(time) {
        min = (time / 60);
        if(min >= 1) return Math.round(min);
        return 0;
    }
    ...
    function getConvertSecond(time) {
        sec = (time % 3600);
        sec = (sec % 60);
        if(sec >= 1) return Math.round(sec);
        return 0;
    }
